-- Quick Fix: Remove BM25 WHERE clause to allow vector-only matches
-- This allows the hybrid search to return products found by semantic similarity
-- even if they don't contain the exact search term

-- Drop and recreate the hybrid_product_search function
...
CREATE OR REPLACE FUNCTION hybrid_product_search(
  ...
  vector_weight NUMERIC DEFAULT 0.65,
  bm25_weight NUMERIC DEFAULT 0.35
)
RETURNS TABLE (
  id UUID,
  product_name TEXT,
...
BEGIN
  ...
  bm25_search AS (
    ...
    WHERE
      ...
      AND (category_filter IS NULL OR p.category_name ILIKE category_filter)
      AND p.active = TRUE
      -- REMOVED: The WHERE clause that required text matching
      -- This allows products to have bm25_score = 0 and still be included
      -- Products will be ranked by vector similarity only if text doesn't match
  ),
  combined_scores AS (
    SELECT
...
COMMENT ON FUNCTION hybrid_product_search IS 'Hybrid search combining vector similarity and BM25 full-text search. Products without text matches can still be found via vector similarity.';
